...
# 1. Leer los archivos necesarios
try:
    flag_enc = open('level4.flag.txt.enc', 'rb').read()
    correct_pw_hash = open('level4.hash.bin', 'rb').read()
except FileNotFoundError:
    print("Error: Asegúrate de que los archivos 'level4.flag.txt.enc' y 'level4.hash.bin' están en la misma carpeta.")
    exit()

# 2. La lista de contraseñas proporcionada
...
# 3. Fuerza bruta iterando sobre la lista
print("[*] Iniciando crackeo...")
for pw in pos_pw_list:
    # Hasheamos la contraseña actual del bucle
    current_hash = hash_pw(pw)
    
    # Comparamos con el hash correcto
    if current_hash == correct_pw_hash:
        print(f"[+] ¡Éxito! Contraseña encontrada: {pw}")
        
        # Desencriptamos la flag
        decryption = str_xor(flag_enc.decode(), pw)
        print(f"[+] FLAG: {decryption}")
        
        # Rompemos el bucle porque ya hemos ganado
        break
else:
    print("[-] Ninguna contraseña de la lista era la correcta.")
